...
file_buff=pandas.read_csv(os.getcwd()+"/Desktop/400ml_min.csv",usecols=['x', 'y', 'v','u'])
print(file_buff)
...
x_vtk_mesh = np.zeros((np.size(x_buff_mesh [:]),1))
y_vtk_mesh = np.zeros((np.size(x_buff_mesh [:]),1))
#VTKpoints = vtk.vtkPoints()
#for i in range(n_points):
#	pt_iso  =  data_vtk.GetPoint(i)
#	x_vtk_mesh[i] = pt_iso[0]	
...
v_buff_mesh= np.asarray(v_buff_mesh) #convert to numpy 

data_vel_u = u_buff_mesh / U_scale
data_vel_v = u_buff_mesh / U_scale
