...
def geo_train(device,x_in,xb,cb,batchsize,learning_rate,epochs,path,Flag_batch,C_analytical,Vel,Diff):
	...
	C_Result = output.data.cpu().numpy()
...
net2_final_2 = geo_train(device,x,xb,cb,batchsize,learning_rate,epochs,path,Flag_batch,C_analytical,Vel/2,Diff )
net2_final_3 = geo_train(device,x,xb,cb,batchsize,learning_rate,epochs,path,Flag_batch,C_analytical,0,Diff )
plt.show()
#tic = time.time()
